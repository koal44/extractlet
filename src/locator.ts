...
export const asAbsUrl: ValMapFn = (v, doc) => {
  ...
  catch { return warnReturn(v, '[asAbsUrl] invalid URL:', v); }
};

export const asIdFrag: ValMapFn = (v) => {
  if (!v.includes('#')) return warnReturn(v, '[asIdFrag] no fragment in:', v);
  const frag = v.split('#')[1];
  return frag ? frag : warnReturn(v, '[asIdFrag] empty fragment in:', v);
};

export const asLastPathSeg: ValMapFn = (v) => {
  const seg = v.split('/').filter(Boolean).pop();
  return seg ?? warnReturn(v, '[asLastPathSeg] no path segment in:', v);
};
