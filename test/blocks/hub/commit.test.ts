...
import { blocks } from '../../../src/sites/hub/pages/commit';
...
describe('github commit page blocks', () => {
  describe('attribution', () => {
    ...
    const attributionBlock = mainBlock.fields?.find((b) => b.name === 'attribution');
    expect(attributionBlock).toBeDefined();
    if (!attributionBlock) return;
    ...
    it('should have proper spacing around the author names', () => {
      ...
      const [attribution] = extractBlocks(el(`<div>${html}</div>`), [attributionBlock], ctxs);
