...
import { toMdCtx as seToMdCtx, toHtmlCtx as seToHtmlCtx, scrapePostContributor } from '../../src/se.js';
...
test('toHtml_test1', () => {
  ...
  const result = toHtml(el(html), seToHtmlCtx);
  const expected = el(`
    <div>
      <p>
        <a href="http://en.wikipedia.org/wiki/Tensor">Wikipedia</a> says that a linear transformation is a
...
test('toMd_converts_spoiler_blockquote_to_markdown', () => {
  ...
  const result = toMd(el(html), seToMdCtx);
